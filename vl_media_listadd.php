...
$vl_media_list_add = NULL; // Initialize page object first

class cvl_media_list_add extends cvl_media_list {
	...
	// Page object name
	var $PageObjName = 'vl_media_list_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["vl_media_id"] != "") {
				...
				$this->setKey("vl_media_id", $this->vl_media_id->CurrentValue); // Set up key
			} else {
				$this->setKey("vl_media_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "vl_media_listview.php")
	...
	function LoadDefaultValues() {
		$this->vl_media_type->CurrentValue = 1;
		$this->vl_media_name->CurrentValue = NULL;
		$this->vl_media_name->OldValue = $this->vl_media_name->CurrentValue;
		$this->vl_media_file->Upload->DbValue = NULL;
		$this->vl_media_file->OldValue = $this->vl_media_file->Upload->DbValue;
		$this->vl_media_file->CurrentValue = NULL; // Clear file related field
		$this->vl_media_file_custom->Upload->DbValue = NULL;
		$this->vl_media_file_custom->OldValue = $this->vl_media_file_custom->Upload->DbValue;
		$this->vl_media_file_custom->CurrentValue = NULL; // Clear file related field
	}
	...
	function RenderRow() {
		...
		$this->vl_media_file_custom->ViewCustomAttributes = "";
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->vl_media_file);
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->vl_media_file_custom);
	...
	function AddRow($rsold = NULL) {
		...
		// vl_media_type
		$this->vl_media_type->SetDbValueDef($rsnew, $this->vl_media_type->CurrentValue, NULL, FALSE);

		// vl_media_name
		$this->vl_media_name->SetDbValueDef($rsnew, $this->vl_media_name->CurrentValue, NULL, FALSE);

		// vl_media_file
		if ($this->vl_media_file->Visible && !$this->vl_media_file->Upload->KeepFile) {
			$this->vl_media_file->Upload->DbValue = ""; // No need to delete old file
		...
		if ($this->vl_media_file_custom->Visible && !$this->vl_media_file_custom->Upload->KeepFile) {
			$this->vl_media_file_custom->Upload->DbValue = ""; // No need to delete old file
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->vl_media_id->setDbValue($conn->Insert_ID());
				$rsnew['vl_media_id'] = $this->vl_media_id->DbValue;
				if ($this->vl_media_file->Visible && !$this->vl_media_file->Upload->KeepFile) {
					if (!ew_Empty($this->vl_media_file->Upload->Value)) {
...
// Create page object
if (!isset($vl_media_list_add)) $vl_media_list_add = new cvl_media_list_add();

// Page init
$vl_media_list_add->Page_Init();

// Page main
$vl_media_list_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$vl_media_list_add->Page_Render();
?>
...
var CurrentForm = fvl_media_listadd = new ew_Form("fvl_media_listadd", "add");

// Validate form
fvl_media_listadd.Validate = function() {
...
fvl_media_listadd.Form_CustomValidate = 
...
fvl_media_listadd.ValidateRequired = true;
<?php } else { ?>
fvl_media_listadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fvl_media_listadd.Lists["x_vl_media_type"] = {"LinkField":"x_vl_media_type_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_vl_media_type_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php $vl_media_list_add->ShowPageHeader(); ?>
<?php
$vl_media_list_add->ShowMessage();
?>
...
<form name="fvl_media_listadd" id="fvl_media_listadd" class="<?php echo $vl_media_list_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($vl_media_list_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $vl_media_list_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $vl_media_list_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fvl_media_listadd.Init();
</script>
<?php
$vl_media_list_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$vl_media_list_add->Page_Terminate();
?>
